...
def test_psa_ranges():
    ...
    # 1. Low Risk (<= 0.3)
    r_low = classify_risk(0.2, GleasonScore.GG1, TumorStage.PT2, 20, PetFindings.NEGATIVE, MarginStatus.R0)
    assert r_low == RiskLevel.LOW, f"PSA 0.2 should be LOW, got {r_low}"
    print("PASS: PSA 0.2 -> Low")
    
    r_low_edge = classify_risk(0.3, GleasonScore.GG1, TumorStage.PT2, 20, PetFindings.NEGATIVE, MarginStatus.R0)
    ...
    r_int = classify_risk(0.4, GleasonScore.GG1, TumorStage.PT2, 20, PetFindings.NEGATIVE, MarginStatus.R0)
    assert r_int == RiskLevel.INTERMEDIATE, f"PSA 0.4 should be INT, got {r_int}"
    print("PASS: PSA 0.4 -> Intermediate")

    r_int_edge = classify_risk(0.7, GleasonScore.GG1, TumorStage.PT2, 20, PetFindings.NEGATIVE, MarginStatus.R0)
    assert r_int_edge == RiskLevel.INTERMEDIATE, f"PSA 0.7 should be INT, got {r_int_edge}"
    print("PASS: PSA 0.7 -> Intermediate")
    
    # 3. High Risk (> 0.7)
    r_high = classify_risk(0.8, GleasonScore.GG1, TumorStage.PT2, 20, PetFindings.NEGATIVE, MarginStatus.R0)
...
def test_pet_not_performed():
    ...
    r_np = classify_risk(0.2, GleasonScore.GG1, TumorStage.PT2, 20, PetFindings.NOT_PERFORMED, MarginStatus.R0)
